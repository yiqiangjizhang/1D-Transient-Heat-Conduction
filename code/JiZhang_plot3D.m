...
% 1.2. Numerical data
N_CV = 100;                    % [units]       % Number of Control Volumes
...
% % 6.1 Save position
% fileID = fopen('data/x_data.txt', 'w');
% for i = 1:size(nodes_pos, 1)
%     for j = 1:size(nodes_pos, 2)
%         fprintf(fileID, "%.10f\t", nodes_pos(i,j));
%     end
%     fprintf(fileID, "\n");
% end
% 
% % 6.2 Save time
% fileID = fopen('data/t_data.txt', 'w');
% for i = 1:size(t, 1)
%     for j = 1:size(t, 2)
%         fprintf(fileID, "%.10f\t", t(i,j));
%     end
%     fprintf(fileID, "\n");
% end
% 
% % 6.3 Save temperatures
% fileID = fopen('data/temp_data.txt', 'w');
% for i = 1:size(T, 1)
%     for j = 1:size(T, 2)
%         fprintf(fileID, "%.10f\t", T(i,j));
%     end
